# GrantNova — Grant Scout & Management System - High-Level Design Document

**Version**: 1.2.0
**Created**: 2026-06-19 15:29 EST
**Modified**: 2026-07-16
**Change from v1.1.0**: Renamed to **GrantNova**. Added the **Grant Scout subsystem** (§14) — discovery pipeline (search/scrape → deterministic extract → pre-review evaluate → staged review → report). Database brought under Alembic control.
...
This document translates the GrantNova PRD v1.1.0 into a technical architecture describing how the system's components are structured, how they interact, and the key design decisions that guide implementation. The central architectural decision in v1.1.0 is the **removal of all paid AI runtime dependencies** from the production system.
...
| Term | Definition |
|------|-----------|
| GrantNova | Grants Manager Assistant System |
...
## 14. Grant Scout Subsystem (Discovery)

The Grant Scout is the discovery/reconnaissance arm added in v1.2.0. It is a pipeline
of small, deterministic, independently-testable components; no paid AI; human-in-the-loop.

**Component map (`src/discovery/`, `src/services/`):**

| Component | Responsibility |
|---|---|
| `sources/*` (grants_gov, web_search, funder_site, aggregator) | HUNT — connectors that turn a query/profile into candidate leads; disabled-safe |
| `query_expander` | HUNT — org profile → ≤40 deterministic queries ("search everywhere") |
| `fetcher.PoliteFetcher` | GATHER — robots.txt, per-host rate limit, descriptive UA, disk cache |
| `extractor` | GATHER — deterministic HTML/text → fields + per-field confidence + evidence |
| `evaluator.evaluate_candidate` | EVALUATE — reuses ScoringEngine + deadline classifier → fit, eligibility, strong-match, Act Now |
| `DiscoveredCandidate` (model) + migration 004 | staging table; fit/eligibility/urgency columns |
| `DiscoveryService` | orchestrates search → stage → evaluate → dedup-flag → ranked queue → human-confirmed import |
| `ScoutReportService` + `scout_digest.md.j2` | REPORT — read-only weekly intelligence digest |
| `pages/09_discover.py` (🛰️ Grant Scout) | UI — search, ranked green queue, report, scrape URL, CSV import |

**Data flow:** `run_search`/`run_full_sweep` → connectors fetch (polite) → extractor
(deterministic + confidence) → `evaluate_candidate` (fit + eligibility + urgency) →
`DiscoveredCandidate` staged (status NEW, dedup-flagged) → ranked review queue →
human confirms → `import_candidate` → `GrantService.create_grant` (existing pipeline).
Nothing auto-imports. `ScoutReportService` reads the queue to produce the digest.

**Schema authority:** Alembic is authoritative for the real database (migrations
001→004); `create_all` is used only for building fresh test DBs. A drift-guard test
enforces ORM/migration parity. See `05_Grant_Scout_Task_Implementation_Plan_v1.0.0.md`
for full acceptance criteria and `DISAMBIGUATION_RECORD_v1.0.md` D-016–D-024.
