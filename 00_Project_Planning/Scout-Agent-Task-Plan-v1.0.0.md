...
version: 1.0.0
supersedes: Scout-Agent-Task-Plan-v0.5.0-ROUGH-PROMPT.md
created: 2026-07-17
...
# Grant Scout — Agent Task Plan v1.0.0
...
This v1.0.0 turns that into a structured plan **grounded in current reality**: the Grant
Scout module is already **built and green** (7 commits on `feat/grantnova-scout`, full
suite **174 passing**, DB at Alembic head 004). So this is not a greenfield build — it is
a **certify → close-gaps (TDD) → review** plan. It:
...
> **Honest note on skills in this environment:** the Superpowers *skills* are installed
> but currently **not invokable by name** here (the `Skill` tool returns "Unknown skill",
> and detached background agents can't write/execute in this sandbox). So this plan applies
> each skill's **method** with the orchestrator (me) driving directly and running every
> verification command, rather than dispatching unattended subagents. If/when the skills
> register and a sandbox for agents is approved, §4 tasks can be handed to real subagents
> unchanged.
...
| Phase | Superpowers skill (method applied) | Output |
|---|---|---|
| 1. Audit | writing-plans + the plan's own AC discipline | Requirements Traceability Matrix; gap list |
| 2. Decompose | **subagent-driven-development** | Discrete agent tasks with roles, inputs, AC, VERIFY |
| 3. Execute | **test-driven-development** | Failing test first → minimal code → green → refactor |
| 4. Certify | **requesting-code-review** | Reviewed diff `main..feat/grantnova-scout`; findings triaged |
| (gate) | **verification-before-completion** | Orchestrator re-runs every VERIFY before "done" |
...
- **G3 (correctness risk, from the handoff):** the **seeded** active `SelectionCriteria`
  may not be in the scoring engine's `config` format — if so, every candidate scores 0.0
  and FR-SCOUT-301/304 are effectively dead in production even though unit tests pass on
...
Three self-contained tasks, each written so it *could* be handed to an implementer subagent
(role, inputs, constraints, acceptance criteria, VERIFY). In this environment the
orchestrator executes them directly; the structure is unchanged.
...
- **Role:** implementer (TDD).
- **Files:** `tests/unit/test_scout_queue_label.py` (new); if the label logic is inline in
  the Streamlit page, extract a pure helper `queue_label(candidate)` into a testable module
  (small refactor) so it can be asserted without a UI runtime.
...
**Dependencies:** T1, T2, T3 are independent and may run in any order. All are additive
(new test files; T2/T3 may touch one production file each).
...
1. **RED** — write the test expressing the requirement; run it; **confirm it fails** for the
   right reason (paste the failure).
2. **GREEN** — write the minimum code to pass (for T1/T2 usually none or a tiny extraction;
   for T3, the seed fix only if RED proves a real defect).
...
Once Phase 3 is green, request a code-review over the **entire Scout diff**
`main..feat/grantnova-scout` (or the local `/code-review` skill at high effort). The review
targets **quality, not new features**:
...
same TDD loop (§5) and re-verify; decisions logged. Optionally apply the
**receiving-code-review** method to vet each suggestion rather than accept blindly.
...
- **Background agents** stay in a strict per-project sandbox once enabled; until then the
...
2. **Review engine for Phase 4:** the local `/code-review` skill (available now) vs. the
   Superpowers `requesting-code-review` skill (blocked until skills register). Recommend
   `/code-review` at high effort now; swap later if desired.
3. **T2 refactor:** OK to extract the queue-label helper out of the Streamlit page into a
   testable module? (Recommended — enables a real unit test.)
